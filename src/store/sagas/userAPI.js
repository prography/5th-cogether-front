...
import { LOGIN_REQUEST, LOGIN_FAIL, loginSuccessAction, LOGOUT_REQUEST, LOGOUT_FAIL, logoutSuccessAction,
    JOIN_REQUEST, JOIN_FAIL, joinSuccessAction  } from "../actions/getUser";
...
function* getLoginData({payload}) {
    try {
        ...
        const responseBody = yield call(
            [axios, "post"],
            "https://cogether.azurewebsites.net/accounts/api-token-auth/",
            json
        );
...
function* getJoinData({payload}) {
    try {
        ...
        const responseBody = yield call(
            [axios, "post"],
            "https://cogether.azurewebsites.net/accounts/",
            json
        );
